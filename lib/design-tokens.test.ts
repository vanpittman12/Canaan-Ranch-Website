...
describe("site builder visual lock", () => {
  ...
  it("uses a substantiated Tier 1 / conservation-easement hero line without a county", () => {
    ...
    expect(brand.heroLead).toContain("a review before anything closes");
    expect(brand.heroLead).not.toContain("human review");
    ...
    expect(intakePage).toContain("Dates are set automatically when you");
    expect(intakePage).not.toContain("seller-side");
    expect(intakePage).not.toContain("typed into the Word agreement and DocuSign");
    ...
    expect(intakeForm).not.toContain("Appears as Its:");
    expect(intakeForm).not.toContain("Paragraph 2");
    expect(intakeForm).not.toContain("Maps to");
    expect(intakeForm).not.toContain("First part of");
    expect(intakeForm).not.toContain("Second part of");
    expect(intakeForm).not.toContain("when the Word file has a blank");
    expect(intakeForm).not.toContain("Appended as");
    expect(intakeForm).toContain("Shows as your project name on the agreement.");
    expect(intakeForm).toContain("Submit & download agreement");
    expect(intakeForm).toContain("Update & download agreement");
    expect(intakeForm).toContain("Creating your agreement…");
    expect(intakeForm).toContain("Saving…");
    expect(intakeForm).not.toContain("Generate populated agreement");
    expect(intakeForm).not.toContain("Submitting drafts the");
    expect(intakeForm).toContain("Already on file");
    expect(intakeForm).not.toContain("Seller-side, already on file");
    expect(landing).not.toContain("no on-screen contract preview");
